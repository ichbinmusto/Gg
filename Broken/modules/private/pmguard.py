...
from Broken.database.pmpermitdb import get_approved_users, pm_guard
import Broken.database.pmpermitdb as Broken
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    await Broken.set_limit(int(arg))
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Broken.set_block_message(Broken.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await Broken.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await Broken.get_pm_settings()
    await Broken.allow_user(chat_id)
...
async def deny(client, message):
    chat_id = message.chat.id
    await Broken.deny_user(chat_id)
