...
DEVS = [1841914911, 6369672953]
from Broken.helper.PyroHelpers import get_ub_chats
from Broken.modules.basic.profile import extract_user, extract_user_and_reason
from Broken.database import gbandb as Broken
from Broken.database import gmutedb as Gmute
...
async def gban_user(client: Client, message: Message):
    ...
    if (await Broken.gban_info(user.id)):
    ...
    for gokid in f_chats:
        try:
            ...
            er += 1
    await Broken.gban_user(user.id)
    ok.append(user.id)
    msg = (
        r"**\\#GBanned_User//**"
...
async def ungban_user(client: Client, message: Message):
    ...
    try:
        if not (await Broken.gban_info(user.id)):
        ...
        for good_boi in ung_chats:
            try:
                ...
                er += 1
        await Broken.ungban_user(user.id)
        msg = (
            r"**\\#UnGbanned_User//**"
            f"\n\n**First Name:** [{user.first_name}](tg://user?id={user.id})"
            f"\n**User ID:** `{user.id}`"
...
if ok:
 ...
 async def globals_check(client: Client, message: Message):
    ...
    if not user_id:
        return
    if (await Broken.gban_info(user_id)):
        try:
